...
use spotify::rspotify::ClientError;
...
use spotify::rspotify;
use tidal::Tidal;

type Spotify = spotify::Spotify<rspotify::ClientCredsSpotify>;
...
fn err_is_status_code(e: &anyhow::Error, expected: u16) -> bool {
    for err in e.chain() {
        if let Some(ClientError::Http(http_err)) = err.downcast_ref()
            && let rspotify::http::HttpError::StatusCode(code) = http_err.as_ref()
...
pub async fn get_release_year(
    ...
    loop {
        match tidal.get_album(&artist, &album).await {
            Ok(Some(serenity_command_handler::album::Album {
...
pub async fn get_release_years<'a, I: IntoIterator<Item = (&'a str, &'a str, usize)>>(
    ...
    albums.into_iter().enumerate().for_each(|(i, ab)| {
        ...
        write!(
            &mut query,
            "(lower('{}'), lower('{}'), {})",
            serenity_command_handler::db::escape_str(ab.0),
            serenity_command_handler::db::escape_str(ab.1),
            ab.2
        )
        .unwrap();
    });
