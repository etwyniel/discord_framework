...
use serenity_command_handler::{
    RegisterableModule, command_context::get_str_opt_ac, db::Db, prelude::*,
};
     ...
     WHERE guild_id = ?1 AND quote_number = ?2",
            ...
            |row| {
                ...
                Ok(Quote {
                    ...
                    contents: serenity_command_handler::db::column_as_string(row.get_ref(6)?)?,
                    image: row.get(7)?,
                    attachments: vec![],
                })
            },
...
pub async fn quotes_markov_chain(
    ...
    stmt.query(params![guild_id, user])?
        .map(|row| serenity_command_handler::db::column_as_string(row.get_ref(0)?))
